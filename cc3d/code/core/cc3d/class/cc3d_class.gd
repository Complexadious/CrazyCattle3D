...
func load_level(internal_level_name: String) -> void:
	...
	clear_menus()
	clear_levels()
	var path = Registry.LevelCodePath+internal_level_name+"/"+Registry.LevelCodeTscnPath+internal_level_name+".tscn"
...
func load_menu(internal_menu_name: String, clear_level: bool = true) -> void:
	...
	if clear_level:
		clear_levels()
	var path = Registry.MenuCodePath+internal_menu_name+"/"+Registry.MenuCodeTscnPath+internal_menu_name+".tscn"
	if !ResourceLoader.exists(path):
...
#	__log.call("spawning sheep at " + str(pos), "INFO")
	var sheep = load(Registry.EntityCodePath+"sheep/characternode.tscn").instantiate()
